      ...
      <div class="container">
        <div class="checkout-addr">
          ...
          <div class="addr-list-wrap">
            <div class="addr-list">
              <ul>
                <li v-for="item of addressList" :class="{'check':item.isDefault}" @click="check(item)" ref="item">
                  <dl>
                    <dt>{{item.userName}}</dt>
...
<script>
  ...
  export default {
    ...
    methods: {
        ...
        axios.get("/users/cart").then(res => {
          if (res.status === 200) {
            if (res.data.status === 1) {
              ...
              this.addressList = res.data.result.data.addressList
      ...
      setDefault(item) {
        let newAddId = item.addressId,
          preDefAdd = this.addressList.find(item => {
            return item.isDefault
          })
        axios.post("/users/setdefault", {
          params: {
            preAddId: preDefAdd.addressId,
            newAddId
          }
        })
          .then(res =>{
